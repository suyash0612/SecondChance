...
import { C, S, F, eventColor, eventLabel, colorOpacity } from "../lib/theme";
...
export function TimelineCard({ event, first, last, onPress }: { event: TimelineEvent; first?: boolean; last?: boolean; onPress?: () => void }) {
  ...
  return (
    <View style={s.row}>
      ...
      <Card style={[s.eCard, event.isMilestone && { borderColor: colorOpacity('err', 19), borderWidth: 1.5 }]} onPress={onPress}>
        <View style={s.tags}>
...
const s = StyleSheet.create({
  ...
  pulse: { position: "absolute", top: -4, left: -4, width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: colorOpacity('err', 25) },
